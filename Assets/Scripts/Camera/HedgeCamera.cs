...
public class HedgeCamera : MonoBehaviour
{
    ...
    void CameraMovement()
    {
        ...
        if (!Locked)
        {
            ...
            x += (_joystick.Horizontal * (InputXSpeed * SensiX) * InvertedX) * Time.deltaTime;
            y -= (_joystick.Vertical * (InputYSpeed * SensiY) * InvertedY) * Time.deltaTime;
        }
        else
        {
    ...
    void CameraRotation()
    {
        if (UseAutoRotation && !Locked)
        {
            if (!UseCurve)
            {
                ...
                x += ((_joystick.Horizontal * NormalMod) * AutoXRotationSpeed) * Time.deltaTime;
                y -= 0;
                z = 0;
            }
            else
            {
                ...
                CurveX = CurveX * 100;
                x += ((_joystick.Horizontal * CurveX) * AutoXRotationSpeed) * Time.deltaTime;
                y -= 0;
                z = 0;
            }
        }
